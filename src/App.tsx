...
export default function App() {
  ...
  const isValidPlacement = useCallback((x: number, y: number): boolean => {
    for (let i = 0; i < GAME_PATH.length - 1; i++) {
      if (pointToSegmentDistance({ x, y }, GAME_PATH[i], GAME_PATH[i + 1]) < 35) return false;
    }
  ...
  const handleUseItem = useCallback((type: ItemType) => {
    ...
    if (item.canUse(coins, now)) {
      ...
      item.execute(mosquitoesRef.current, addParticles, soundSystemRef.current, setLives, addFloatingText);
      setCurrentTime(now);
    }
  ...
  const startNextWave = useCallback(() => {
    ...
    setEnemiesRemaining(pattern?.composition.reduce((sum, c) => sum + c.count, 0) || 0);
  }, [isWaveActive, wave]);
  ...
  const saveGame = useCallback(() => {
    const success = SaveLoadSystem.save(coins, lives, wave, towersRef.current, itemsRef.current);
    if (success) {
  ...
  useEffect(() => {
    ...
    const gameLoop = (time: number) => {
      ...
      if (!gameOverRef.current && !gameWonRef.current) {
        ...
        // Check wave end
        if (isWaveActiveRef.current && waveSystemRef.current.allSpawned && mosquitoesRef.current.length === 0) {
          setIsWaveActive(false);
          setEnemiesRemaining(0);
          setWave(prev => {
            if (prev >= 10) {
    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-slate-950 text-white font-sans">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4">
        ...
        <div className="relative border-2 border-slate-800 rounded-2xl overflow-hidden shadow-2xl bg-slate-900">
          <GameCanvas canvasRef={canvasRef} onCanvasClick={handleCanvasClick} />
          
          <WaveIndicator pattern={currentWavePattern} isWaveActive={isWaveActive} />
      ...
      <div className="w-full lg:w-96 bg-slate-900 border-l border-slate-800 p-6 flex flex-col gap-6">
        <div>
          ...
          <p className="text-xs text-slate-400">สถาปัตยกรรม OOP + Retry Wave System</p>
        </div>
        ...
        <div className="mt-auto bg-slate-950/60 p-4 rounded-xl border border-slate-800/60 text-xs text-slate-400 space-y-2">
          ...
          <ul className="list-disc list-inside space-y-1">
            <li>รองรับ <code className="text-emerald-400">Retry Wave</code> ที่แพ้โดยไม่ต้องเริ่มใหม่</li>
            <li>รองรับ <code className="text-emerald-400">Save / Load</code> ผ่าน LocalStorage</li>
            <li>ระบบ <code className="text-amber-400">Counter-Play</code> แก้ทางกัน</li>
          </ul>
        </div>
      </div>
    </div>
